...
class TestExpandMultipleChoice:
    ...
    def test_basic_expansion(self):
        ...
        df = pd.DataFrame(
            {"id": [1, 2, 3], "colors": ["Blue,Green", "Red", "Blue,Red,Yellow"]}
        )
    ...
    def test_keep_original(self):
        ...
        df = pd.DataFrame({"id": [1, 2], "colors": ["Blue", "Red"]})
    ...
    def test_custom_prefix(self):
        ...
        df = pd.DataFrame({"id": [1], "colors": ["Blue"]})
    ...
    def test_missing_values(self):
        ...
        df = pd.DataFrame({"id": [1, 2, 3], "colors": ["Blue", np.nan, "Red"]})
...
class TestCollapseMultipleChoice:
    ...
    def test_basic_collapse(self):
        ...
        result = reshape.collapse_multiple_choice(
            df, ["colors_Blue", "colors_Red", "colors_Green"], "colors"
        )
    ...
    def test_keep_original_columns(self):
        ...
        df = pd.DataFrame({"id": [1], "opt_A": [1], "opt_B": [0]})

        result = reshape.collapse_multiple_choice(
            df, ["opt_A", "opt_B"], "options", drop_original=False
        )
    ...
    def test_no_selection(self):
        ...
        df = pd.DataFrame({"id": [1], "colors_Blue": [0], "colors_Red": [0]})

        result = reshape.collapse_multiple_choice(
            df, ["colors_Blue", "colors_Red"], "colors"
        )
    ...
    def test_custom_separator(self):
        """Test custom separator."""
        df = pd.DataFrame({"id": [1], "opt_A": [1], "opt_B": [1]})

        result = reshape.collapse_multiple_choice(
            df, ["opt_A", "opt_B"], "options", sep=";"
        )
...
class TestWideToLong:
    ...
    def test_basic_wide_to_long(self):
        ...
        df = pd.DataFrame({"id": [1, 2], "Q1_A": [1, 0], "Q1_B": [0, 1]})
    ...
    def test_custom_column_names(self):
        ...
        df = pd.DataFrame(
            {"id": [1, 2], "score_math": [90, 85], "score_reading": [88, 92]}
        )

        result = reshape.wide_to_long(df, stub="score", i="id", j="subject")
...
class TestLongToWide:
    ...
    def test_basic_long_to_wide(self):
        ...
        result = reshape.long_to_wide(
            df, index="id", columns="question", values="response"
        )
    ...
    def test_fill_missing_values(self):
        ...
        df = pd.DataFrame(
            {"id": [1, 2, 2], "question": ["Q1", "Q1", "Q2"], "response": [5, 3, 4]}
        )

        result = reshape.long_to_wide(
            df, index="id", columns="question", values="response", fill_value=0
        )
